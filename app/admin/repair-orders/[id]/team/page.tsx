...
const payMethods = [
  ["sold_hour", "Per sold hour"], ["actual_hour", "Per actual hour"], ["fixed", "Fixed per service"],
  ["labor_percent", "% labor revenue"], ["revenue_percent", "% service revenue"], ["manual", "Manual"], ["none", "No pay"],
] as const;
...
function qty(value: unknown) { const number = Number(value || 0); return Number.isInteger(number) ? String(number) : number.toFixed(3).replace(/0+$/, "").replace(/\.$/, ""); }
...
function assignmentEarning(assignment: Assignment, laborRevenue: number, lineRevenue: number, soldHours: number, actualHours: number) {
  const credit = Number(assignment.credit_percent || 0) / 100;
  const creditedSold = assignment.sold_hours_credit === null ? soldHours * credit : Number(assignment.sold_hours_credit);
  const rate = Number(assignment.compensation_rate || 0);
  switch (assignment.compensation_method) {
    case "sold_hour": return creditedSold * rate;
...
export default async function ROTeamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const jobIds = jobIdsResult.data?.map((row) => row.id) || ["00000000-0000-0000-0000-000000000000"];
  const [{ data: roData }, { data: jobsData }, { data: workersData }, { data: assignmentsData }, { data: itemsData }, { data: requestsData }, { data: timeData }] = await Promise.all([
    ...
    <main className="admin-page workforce-page ro-team-page">
      <div className="shell">
        <header className="admin-header compact-admin-header">
          <div><div className="eyebrow">{ro.ro_number} · Team & Parts</div><h1>{vehicle}</h1><p className="section-copy">{ro.customers?.full_name || "Customer"} · Assign production, track actual time, and manage every requested or billable part.</p></div>
          <div className="admin-account"><Link className="button secondary" href={`/admin/repair-orders/${id}`}>← Full RO</Link><Link href="/admin/people">People & production →</Link></div>
        </header>

        <section className="ro-team-legend admin-panel">
          <div><strong>Production credit</strong><span>How much of this line's revenue/cost is attributed to each person. Multiple workers can split 70/30, 50/50, etc.</span></div>
          <div><strong>Sold hours credit</strong><span>Defaults from billable labor × production %. Override when two people split labor differently.</span></div>
          <div><strong>Actual time</strong><span>Comes from each technician's Start / Stop timer and never changes customer pricing.</span></div>
        </section>

        <section className="ro-team-lines">
          {jobs.map((job) => {
            const lineItems = items.filter((item) => item.ro_job_id === job.id);
            const laborRevenue = lineItems.filter((i) => i.item_type === "labor").reduce((sum, i) => sum + Number(i.quantity) * Number(i.unit_price), 0);
            const soldHours = lineItems.filter((i) => i.item_type === "labor").reduce((sum, i) => sum + Number(i.quantity), 0);
            const lineRevenue = lineItems.reduce((sum, i) => sum + (i.item_type === "discount" ? -Math.abs(Number(i.quantity) * Number(i.unit_price)) : Number(i.quantity) * Number(i.unit_price)), 0);
            const partsCost = lineItems.filter((i) => i.item_type === "part").reduce((sum, i) => sum + Number(i.quantity) * Number(i.unit_cost || 0), 0);
            ...
            return (
              <article className="admin-panel ro-team-line" key={job.id}>
                <header className="ro-team-line-head"><div><span className="ro-line-number">Line {job.line_number}</span><h2>{job.title || "Untitled service"}</h2><p>{job.customer_concern || "No concern entered"}</p></div><div><span className={`status-pill status-${job.work_status}`}>{titleCase(job.work_status)}</span><strong>{money(lineRevenue)}</strong></div></header>
                <div className="line-production-strip">
                  <div><span>Line revenue</span><strong>{money(lineRevenue)}</strong></div><div><span>Sold labor</span><strong>{soldHours.toFixed(1)} hr</strong></div><div><span>Direct parts cost</span><strong>{money(partsCost)}</strong></div><div><span>Unassigned credit</span><strong>{Math.max(0,100-usedCredit).toFixed(0)}%</strong></div>
                </div>

                <div className="ro-team-columns">
                  <section>
                    <div className="tech-card-heading"><span>People on this line</span><strong>{lineAssignments.length}</strong></div>
                    {lineAssignments.length === 0 ? <p className="admin-muted">No one assigned yet. Tech accounts cannot see this line until assigned.</p> : null}
                    {lineAssignments.map((assignment) => {
                      ...
                      const earned = assignmentEarning(assignment, laborRevenue, lineRevenue, soldHours, actualHours);
                      const production = lineRevenue * Number(assignment.credit_percent) / 100;
                      const directCost = partsCost * Number(assignment.credit_percent) / 100;
                      const contribution = production - directCost - earned;
                      return (
                        <details className="assignment-detail" key={assignment.id} open={lineAssignments.length === 1}>
                          <summary><span><strong>{worker?.display_name || "Worker"}</strong><small>{assignment.credit_percent}% credit · {actualHours.toFixed(1)} actual hr</small></span><span><b>{money(earned)} earned</b><small>{money(contribution)} contribution</small></span></summary>
                          <form action={saveAssignment} className="admin-form-grid compact-item-form">
                            <input type="hidden" name="repairOrderId" value={id} /><input type="hidden" name="jobId" value={job.id} /><input type="hidden" name="workerId" value={assignment.worker_id} />
                            <div className="field"><label>Production credit %</label><input name="creditPercent" type="number" min="0" max="100" step="0.1" defaultValue={assignment.credit_percent} disabled={Boolean(assignment.pay_period_id)} /></div>
                            <div className="field"><label>Sold hours credit</label><input name="soldHoursCredit" type="number" min="0" step="0.1" defaultValue={assignment.sold_hours_credit ?? ""} placeholder="Auto" disabled={Boolean(assignment.pay_period_id)} /></div>
                            <div className="field"><label>Pay method</label><select name="compensationMethod" defaultValue={assignment.compensation_method} disabled={Boolean(assignment.pay_period_id)}>{payMethods.map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></div>
                            <div className="field"><label>Rate</label><input name="compensationRate" type="number" min="0" step="0.01" defaultValue={assignment.compensation_rate} disabled={Boolean(assignment.pay_period_id)} /></div>
                            <div className="field"><label>Manual pay</label><input name="manualPay" type="number" min="0" step="0.01" defaultValue={assignment.manual_pay ?? ""} disabled={Boolean(assignment.pay_period_id)} /></div>
                            <div className="field"><label>Calculated pay</label><div className="calculated-field">{money(earned)}</div></div>
                            {!assignment.pay_period_id ? <div className="field full"><button className="button secondary" type="submit">Update assignment</button></div> : <p className="admin-muted field full">Locked in a pay period.</p>}
                          </form>
                          {!assignment.pay_period_id ? <form action={removeAssignment} className="assignment-remove-row"><input type="hidden" name="assignmentId" value={assignment.id} /><input type="hidden" name="repairOrderId" value={id} /><span>Remove {worker?.display_name || "worker"} from this line</span><button className="text-button danger" type="submit">Remove</button></form> : null}
                        </details>
                      );
                    })}
                    ...
                    <details className="inline-disclosure" open={lineAssignments.length === 0}>
                      ...
                      <form action={saveAssignment} className="admin-form-grid compact-item-form">
                        ...
                        <div className="field"><label>Sold hours credit override</label><input name="soldHoursCredit" type="number" min="0" step="0.1" placeholder="Auto" /></div>
                  ...
                  <section>
                    <div className="tech-card-heading"><span>Parts requested</span><strong>{lineRequests.length}</strong></div>
                    {lineRequests.length === 0 ? <p className="admin-muted">No technician parts requests.</p> : null}
                    <div className="parts-request-owner-list">
                      {lineRequests.map((request) => (
                        <details className="part-request-owner" key={request.id} open={request.status === "requested"}>
                          <summary><span><strong>{qty(request.quantity)} {request.unit} · {request.description}</strong><small>{request.part_number || "No part #"}{request.notes ? ` · ${request.notes}` : ""}</small></span><span className={`status-pill status-${request.status}`}>{titleCase(request.status)}</span></summary>
                          <form action={updatePartRequestStatus} className="inline-status-form"><input type="hidden" name="repairOrderId" value={id} /><input type="hidden" name="requestId" value={request.id} /><select name="status" defaultValue={request.status}>{partStatuses.map((status) => <option key={status} value={status}>{titleCase(status)}</option>)}</select><button className="button secondary" type="submit">Update status</button></form>
                          <form action={convertPartRequest} className="admin-form-grid compact-item-form">
                            ...
                            <div className="field"><label>Quantity</label><input name="quantity" type="number" min="0.001" step="0.001" defaultValue={request.quantity} /></div>
                            ...
                            <div className="field"><label>After conversion</label><select name="nextStatus" defaultValue="sourced"><option value="sourced">Sourced</option><option value="ordered">Ordered</option><option value="received">Received</option></select></div>
                ...
                <details className="line-section" open={pricedParts.length > 0}>
                    ...
                    {pricedParts.map((part) => (
                      <form action={updateTrackedPart} className="tracked-part-card" key={part.id}>
                        ...
                        <div className="admin-form-grid compact-item-form">
                          <div className="field"><label>Description</label><input name="description" defaultValue={part.description} /></div><div className="field"><label>Part #</label><input name="partNumber" defaultValue={part.part_number || ""} /></div>
                          <div className="field"><label>Qty</label><input name="quantity" type="number" min="0.001" step="0.001" defaultValue={part.quantity} /></div><div className="field"><label>Unit</label><select name="unit" defaultValue={part.unit || "ea"}>{units.map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select></div>
                          <div className="field"><label>Unit cost</label><input name="unitCost" type="number" min="0" step="0.01" defaultValue={part.unit_cost ?? ""} /></div><div className="field"><label>Unit price</label><input name="unitPrice" type="number" min="0" step="0.01" defaultValue={part.unit_price} /></div>
                  ...
                  <details className="inline-disclosure">
                    ...
                    <form action={addTrackedPart} className="admin-form-grid compact-item-form">
                      <input type="hidden" name="repairOrderId" value={id} /><input type="hidden" name="jobId" value={job.id} />
                      <div className="field"><label>Description</label><input name="description" placeholder="Oil filter seal" /></div><div className="field"><label>Part #</label><input name="partNumber" /></div>
                      <div className="field"><label>Quantity</label><input name="quantity" type="number" min="0.001" step="0.001" defaultValue="1" /></div><div className="field"><label>Unit</label><select name="unit" defaultValue="ea">{units.map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select></div>
                      <div className="field"><label>Unit cost</label><input name="unitCost" type="number" min="0" step="0.01" /></div><div className="field"><label>Unit price</label><input name="unitPrice" type="number" min="0" step="0.01" /></div>
                      <div className="field"><label>Condition</label><select name="partCondition" defaultValue="new_oem"><option value="new_oem">New OEM</option><option value="new_aftermarket">New aftermarket</option><option value="rebuilt">Rebuilt</option><option value="used">Used</option><option value="customer_supplied">Customer supplied</option></select></div><label className="checkbox-label"><input type="checkbox" name="taxable" value="yes" defaultChecked /> Taxable</label>
